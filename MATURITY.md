...
- **Assignment**: Week 4 Assignment — Richardson Maturity Model Evaluating Your REST API
...
To ensure full compliance with Level 2 specifications (specifically RFC 7231 / RFC 9110 regarding resource creation), the `POST /tasks` endpoint was improved to include the `Location` response header:
...
- **Decoupling**: Client applications do not need to hardcode API endpoints (e.g. `/tasks/123`); they follow relational links (e.g. `_links.delete`).
